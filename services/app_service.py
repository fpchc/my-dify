...
class AppService:
    def get_paginate_apps(self, tenant_id: str, args: dict) -> Pagination | None:
        ...
        if args.get("is_hidden"):
            is_hidden = args["is_hidden"]
            filters.append(App.is_hidden == is_hidden)
        if args.get("tag_ids"):
    ...
    def update_app_hidden(self, app: App, is_hidden: str) -> App:
        if is_hidden == app.is_hidden:
            return app

        app.is_hidden = is_hidden
        app.updated_by = current_user.id
        app.updated_at = datetime.now(UTC).replace(tzinfo=None)
        db.session.commit()

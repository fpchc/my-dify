...
def validate_out_app_token(view: Optional[Callable] = None):
    def decorator(view_func):
        ...
        def decorated_view(*args, **kwargs):
            ...
            if not auth_header.startswith("Bearer "):
                raise Unauthorized(
                    "Authorization header must start with 'Bearer' and include a token")

            _, auth_token = auth_header.split(" ", 1)

            if auth_token != dify_config.OUT_SECRET_KEY:
